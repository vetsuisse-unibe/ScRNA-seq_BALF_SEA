...
# load the Seurat object for each cell population
...
# defining cluster colors (1 color = 1 cell type, using different shades for the clusters) 
...
# FIGURE 1B
tiff(file='sc22_featureplots.tiff', width = 3000, height = 4000, units = "px",res = 300)
...
pt_major$horse_id <- ifelse(pt_major$status == "SEA", paste0("C", pt_major$horse), paste0("A", pt_major$horse))
...
# FIGURE 1E
major_boxplot <- ggplot(pt_major, aes(x = cell_type, y = Freq, fill = status)) +
  geom_boxplot(outlier.shape = NA) +
  labs(x = NULL, y = "Cell number", fill = "Group") +
...
major_boxplot <- major_boxplot +
  annotate("segment", x = 0.85, xend = 1.15, y = 500, yend = 500, colour = "black") +
  annotate("text", x = 1, y = 600, label = "***") +
  annotate("segment", x = 4.85, xend = 5.15, y = 1900, yend = 1900, colour = "black") +
  annotate("text", x = 5, y = 2000, label = "***") +
  ylim(-1, 3800)
...
# FIGURE 2B
tiff(file='neu_featureplots.tiff', width = 1500, height = 2500, units = "px",res = 300)
...
# Create a new column 'status' based on 'horse_id'
pt_ne$status <- ifelse(pt_ne$horse %in% c("30", "33", "44","52","56","68"), "Control", "SEA")
# Create a new column 'horse_id' based on 'status'
pt_ne$horse_id <- ifelse(pt_ne$status == "SEA", paste0("A", pt_ne$horse), paste0("C", pt_ne$horse))
# Reorder levels of 'horse_id' to have control horses on the left
...
# FIGURE 2E
ne_boxplot <- ggplot(pt_ne, aes(x = cell_type, y = Freq, fill = status)) +
  geom_boxplot(outlier.shape = NA) +
  labs(x = NULL, y = "Cell number", fill = "Group") +
  scale_fill_manual(values = c("#99CCFF", "#CC3333")) +
  theme_classic(base_size = 15) +
...
ne_boxplot <- ne_boxplot + ylim(0, 420)
...
# Create a new column 'status' based on 'horse_id'
pt_B$status <- ifelse(pt_B$horse %in% c("30", "33", "44","52","56","68"), "Control", "SEA")
# Create a new column 'horse_id' based on 'status'
pt_B$horse_id <- ifelse(pt_B$status == "SEA", paste0("A", pt_B$horse), paste0("C", pt_B$horse))
# Reorder levels of 'horse_id' to have control horses on the left
...
# FIGURE 3E
B_boxplot <- ggplot(pt_B, aes(x = cell_type, y = Freq, fill = status)) +
  geom_boxplot(outlier.shape = NA) +
  labs(x = NULL, y = "Cell number", fill = "Group") +
  scale_fill_manual(values = c("#99CCFF", "#CC3333")) +
  theme_classic(base_size = 15) +
...
B_boxplot <- B_boxplot + 
  annotate("segment", x = 2.85, xend = 3.15, y = 20, yend = 20, colour = "black") +
  annotate("text", x = 3, y = 21, label = "*") +
  ylim(-1, 60)
...
# FIGURE 4B
tiff(file='T_violinplot.tiff', width = 1000, height = 2000, units = "px",res = 300)
...
# FIGURE 4C
tiff(file='T_featureplots.tiff', width = 4000, height = 1500, units = "px",res = 300)
...
# Create a new column 'status' based on 'horse_id'
pt_T$status <- ifelse(pt_T$horse %in% c("30", "33", "44","52","56","68"), "Control", "SEA")
# Create a new column 'horse_id' based on 'status'
pt_T$horse_id <- ifelse(pt_T$status == "SEA", paste0("A", pt_T$horse), paste0("C", pt_T$horse))
# Reorder levels of 'horse_id' to have control horses on the left
...
# FIGURE 4F
T_boxplot <- ggplot(pt_T, aes(x = cell_type, y = Freq, fill = status)) +
  geom_boxplot(outlier.shape = NA) +
  labs(x = NULL, y = "Cell number", fill = "Group") +
  scale_fill_manual(values = c("#99CCFF", "#CC3333")) +
  theme_classic(base_size = 15) +
...
# FIGURE 5B
tiff(file='moma_featureplots.tiff', width = 8000, height = 1500, units = "px",res = 300)
...
# Create a new column 'status' based on 'horse_id'
pt_moma$status <- ifelse(pt_moma$horse %in% c("30", "33", "44","52","56","68"), "Control", "SEA")
# Create a new column 'horse_id' based on 'status'
pt_moma$horse_id <- ifelse(pt_moma$status == "SEA", paste0("A", pt_moma$horse), paste0("C", pt_moma$horse))
# Reorder levels of 'horse_id' to have control horses on the left
...
# FIGURE 5E
moma_boxplot <- ggplot(pt_moma, aes(x = cell_type, y = Freq, fill = status)) +
  geom_boxplot(outlier.shape = NA) +
  labs(x = NULL, y = "Cell number", fill = "Group") +
  scale_fill_manual(values = c("#99CCFF", "#CC3333")) +
  theme_classic(base_size = 15) +
...
# FIGURE 6C
tiff(file='DC_featureplots.tiff', width = 8000, height = 1500, units = "px",res = 300)
...
# Create a new column 'status' based on 'horse_id'
pt_DC$status <- ifelse(pt_DC$horse %in% c("30", "33", "44","52","56","68"), "Control", "SEA")
# Create a new column 'horse_id' based on 'status'
pt_DC$horse_id <- ifelse(pt_DC$status == "SEA", paste0("A", pt_DC$horse), paste0("C", pt_DC$horse))
# Reorder levels of 'horse_id' to have control horses on the left
...
# FIGURE 6F
DC_boxplot <- ggplot(pt_DC, aes(x = cell_type, y = Freq, fill = status)) +
  geom_boxplot(outlier.shape = NA) +
  labs(x = NULL, y = "Cell number", fill = "Group") +
  scale_fill_manual(values = c("#99CCFF", "#CC3333")) +
  theme_classic(base_size = 15) +
...
p1 <- VlnPlot(sc22_T, features = "IL17A", split.by = "disease_state", 
              pt.size = 0.01, split.plot=F, cols = c("#99CCFF", "#CC3333")) &
  theme(axis.title=element_blank(), plot.title=element_text(size=14, face="italic"), legend.position = "none")
p2 <- VlnPlot(sc22_T, features = "IL21", split.by = "disease_state", 
              pt.size = 0.01, split.plot=F, cols = c("#99CCFF", "#CC3333")) &
  theme(axis.title=element_blank(), plot.title=element_text(size=14, face="italic"), legend.position = "none")
p3 <- VlnPlot(sc22_T, features = "CCL20", split.by = "disease_state", 
              pt.size = 0.01, split.plot=F, cols = c("#99CCFF", "#CC3333")) &
  theme(axis.title=element_blank(), plot.title=element_text(size=14, face="italic"), legend.position = "none")

tiff(file='T_DGE.tiff', width = 1000, height = 2000, units = "px",res = 300)
...
(FeaturePlot(sc22_T, features = "percent.ribo") + VlnPlot(sc22_T, features = "percent.ribo") +
    plot_layout(ncol = 2)) & 
...
tiff(file='moma1_vlnplot.tiff', width = dpi*12, height = dpi*5, units = "px",res = dpi)
...
(FeaturePlot(sc22_moma, features = "percent.ribo") + VlnPlot(sc22_moma, features = "percent.ribo") +
    plot_layout(ncol = 2)) & 
  theme(axis.title.x = element_blank(), 
        axis.title.y = element_blank(), legend.position = "none") &
  labs(title="") &
